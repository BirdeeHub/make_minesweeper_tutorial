...
public class ScoreEntry{//This exists because i was converting from string way more times than my brain wanted to do.
    ...
    public ScoreEntry(int x,int y,int bombCount,int lives,int RemainingLives,int time){//<-- this constructor is to be used everywhere else.
    ...
    public String toString(){
        if(validScore){
            return Integer.toString(x)+":"+Integer.toString(y)+":"+Integer.toString(bombCount)+":"+Integer.toString(lives)+"-"+Integer.toString(RemainingLives)+"-"+Integer.toString(time);
        }else return "";
    }
    private boolean setEntryFromWord(String Word){
        ...
        if(currentEntry.length == 3 && boardStrings.length == 4){
            try{
                if(Integer.parseInt(boardStrings[0])<=0||Integer.parseInt(boardStrings[1])<=0||Integer.parseInt(boardStrings[2])<=0||Integer.parseInt(boardStrings[3])<=0||Integer.parseInt(currentEntry[1])<0||Integer.parseInt(currentEntry[2])<0){
                }else{
                    validScore=true;
                    ...
                    this.time=Integer.parseInt(currentEntry[2]);
                }
